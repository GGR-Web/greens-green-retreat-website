...
export default function AdminEditBookingPage() {
  ...
  useEffect(() => {
    async function fetchInitialData() {
        ...
        const cottageResult = await getCottagesForBooking();
         if (cottageResult.cottages) {
            setCottages(cottageResult.cottages);
        } else if (cottageResult.error) {
            toast({ title: "Error", description: `Could not load cottages: ${cottageResult.error}`, variant: 'destructive' });
        }
        ...
        if (bookingResult.booking) {
            ...
            toast({ title: "Error", description: `Could not load booking data: ${bookingResult.error}`, variant: 'destructive' });
  ...
  const fetchBookedDates = useCallback(async (cottageId: string) => {
      ...
      if (result.bookings) {
          const disabledRanges = result.bookings
            .filter(booking => booking.id !== bookingId) // Exclude the current booking
            .map(booking => ({
  ...
  useEffect(() => {
    if (selectedCottageId && !isDataLoading) {
        fetchBookedDates(selectedCottageId);
    }
  }, [selectedCottageId, fetchBookedDates, isDataLoading]);

 async function onSubmit(values: z.infer<typeof adminBookingFormSchema>) {
    setIsLoading(true);
    const result = await updateBooking(bookingId, values);
    setIsLoading(false);

    if (result.success) {
      toast({
          title: "Booking Updated",
          description: `Successfully updated booking ${bookingId}.`,
      });
      router.push(`/admin/bookings`);
    } else {
      toast({
    ...
    <>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update the details for booking ID: {bookingId}</CardDescription>
        </CardHeader>
        <CardContent>
                      ...
                      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              }} value={field.value} disabled={isLoading || cottages.length === 0}>
                                <FormControl>
                                  <SelectTrigger>
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                ...
                                <FormControl>
                                    <Input type="number" placeholder={`Auto: ${calculatedPrice.toLocaleString()}`} {...field} value={field.value ?? ''} onChange={e => field.onChange(e.target.valueAsNumber)} disabled={isLoading} />
                                </FormControl>
                                <FormMessage />
                                </FormItem>
                            )}
