...
/**
 * AI chatbot flow for guest Q&A + lead capture.
 */
...
const createLeadTool = ai.defineTool(
  {
    name: 'createLead',
    description:
      "Use this tool when the user provides their name and email for a follow-up. Saves the user's contact information and the conversation history to the database.",
    inputSchema: z.object({
      name: z.string().describe("The user's full name."),
      email: z.string().describe("The user's email address."),
      conversation: z.string().describe(
        'The recent conversation history with the user, stringified.'
      ),
    }),
...
const AiChatbotOutputSchema = z.object({
  answer: z
    .string()
    .describe(
      "Helpful, on-brand answer to the user's question (or a clarifying question, or a polite request for contact details if the KB lacks the answer)."
    ),
  followUpRequired: z
    .boolean()
    .describe(
      "True ONLY if the answer cannot be derived from the knowledge base; in that case, ask for name and email."
    ),
});
  ...
  async (input) => {
    try {
      ...
      const defaultLeadPrompt =
        "I'm sorry, I can't seem to find the answer to that. I'd be happy to have a member of our team get back to you. What is your name and email address?";
      ...
      return {
        answer:
          "I'm having some technical difficulties at the moment. Please try again shortly.",
        followUpRequired: false,
      };
    }
  }
